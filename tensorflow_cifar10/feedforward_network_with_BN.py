...
epsilon = 1e-5
def fully_connected_layer_with_batch_normalization(inputs, input_dim, output_dim, nonlinearity=tf.nn.relu):
    weights = tf.Variable(
        tf.truncated_normal(
            [input_dim, output_dim], stddev=2. / (input_dim + output_dim)**0.5),
        'weights')
    biases = tf.Variable(tf.zeros([output_dim]), 'biases')
    activation = tf.matmul(inputs, weights) + biases
    batch_mean, batch_var = tf.nn.moments(activation, [0])
    scale = tf.Variable(tf.ones(output_dim))
    beta = tf.Variable(tf.zeros([output_dim]))
    outputs = nonlinearity(tf.nn.batch_normalization(activation, batch_mean, batch_var, beta, scale, epsilon))
    return outputs
...
with tf.name_scope('fc-layer-1'):
    hidden_1 = fully_connected_layer_with_batch_normalization(inputs, train_data.inputs.shape[1], num_hidden)
with tf.name_scope('fc-layer-2'):
    hidden_2 = fully_connected_layer_with_batch_normalization(hidden_1, num_hidden, num_hidden)
...
with tf.name_scope('train'):
    ...
    #train_step = tf.train.AdagradOptimizer(0.001).minimize(error)
